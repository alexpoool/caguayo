...
NEW_DB_URL = (
    f"postgresql+psycopg://{DB_USER}:{DB_PASS}@{DB_HOST}:{DB_PORT}/{NEW_DB_NAME}"
)
...
def create_database():
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(
                text(f"SELECT 1 FROM pg_database WHERE datname = '{NEW_DB_NAME}'")
            )
            if result.fetchone():
        ...
        print(
            "IMPORTANTE: Asegúrate de que la contraseña del usuario 'postgres' sea 'solji'."
        )
