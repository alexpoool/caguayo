...
def generate_log_message(
    method: str, path: str, status_code: int, error_detail: Optional[str] = None
) -> str:
...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
            ...
            if (
                ...
                if response_body:
                    try:
                        ...
                        response_detail = error_json.get("detail") or error_json.get(
                            "message"
                        )
        ...
        log_data = {
            "nivel": "ERROR"
            if status_code >= 500 or error_detail
        ...
        try:
            ...
            error_str = str(e)
            if (
                "no existe la relación" in error_str
                or "UndefinedTableError" in error_str
                or "log" in error_str
            ):
