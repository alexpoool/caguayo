...
async def get_stock_report(
    ...
    return await service.get_stock_por_producto(id_dependencia)
...
async def get_stock_report_pdf(
    ...
    service = ReportesService(db)
    data = await service.get_stock_por_producto(id_dependencia)
    ...
    pdf_buffer = pdf_service.generate_stock_pdf(data, filters)
    
    filename = f"reporte_stock_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
