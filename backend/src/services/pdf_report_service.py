...
class PdfReportService:
    ...
    def __init__(self):
        ...
        self.color_box_border = colors.HexColor('#8DB4E2')    # Borde suave del cuadro azul
    ...
    def _crear_encabezado_estandar(
        ...
        info_rows: List[List[Any]] = [
            [Paragraph(f"<b>EMPRESA:</b> {info_empresa.get('empresa', '')}", self.style_label)],
            [Paragraph(f"<b>DEPENDENCIA:</b> {info_empresa.get('dependencia', '')}", self.style_label)],
            [Paragraph(f"<b>DIRECCIÓN:</b> {info_empresa.get('direccion', '')}", self.style_label)],
        ]
    ...
    def _crear_bloque_firmas(self, page_width: float):
        ...
        t.setStyle(TableStyle([
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ('TOPPADDING', (0, 0), (-1, 0), 20),   # Separación superior del bloque
    ...
    def generate_existencias_pdf(self, data: List[dict], filters: dict):
        ...
        elements.append(self._crear_encabezado_estandar(
            'EXISTENCIAS EN INVENTARIO', info, [fecha], self.PORTRAIT_WIDTH,
        ))
        ...
        t = Table(table_data, colWidths=[1.5 * inch, 4.5 * inch, 1.5 * inch], repeatRows=1)
    ...
    def generate_movimientos_dependencia_pdf(self, data: List[dict], filters: dict):
        ...
        elements.append(self._crear_encabezado_estandar(
            'MOVIMIENTOS DE INVENTARIO', info, rango, self.LANDSCAPE_WIDTH,
        ))
        elements.append(Spacer(1, 15))

        # ── Tabla de datos (7 columnas) ───────────────────────
        headers = ['FECHA', 'CÓDIGO', 'SALDO INICIAL', 'TIPO',
                    'DESCRIPCIÓN', 'CANTIDAD', 'SALDO FINAL']
        table_data: List[List[Any]] = [
        ...
        col_widths = [1.0*inch, 1.1*inch, 1.1*inch, 0.9*inch,
                      3.3*inch, 1.0*inch, 1.1*inch]
    ...
    def generate_movimientos_producto_pdf(self, data: List[dict], filters: dict):
        ...
        elements.append(self._crear_encabezado_estandar(
            'MOVIMIENTOS DE INVENTARIO', info, extras, self.LANDSCAPE_WIDTH,
        ))
        elements.append(Spacer(1, 15))

        # ── Tabla de datos (6 columnas) ───────────────────────
        headers = ['FECHA', 'SALDO INICIAL', 'TIPO',
                    'DESCRIPCIÓN', 'CANTIDAD', 'SALDO FINAL']
        table_data: List[List[Any]] = [
            [Paragraph(f'<b>{h}</b>', self.style_header_cell) for h in headers]
        ]
        ...
        col_widths = [1.2*inch, 1.5*inch, 1.0*inch,
                      3.7*inch, 1.2*inch, 1.4*inch]
        t = Table(table_data, colWidths=col_widths, repeatRows=1)
        style = self._table_style_base() + [
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
