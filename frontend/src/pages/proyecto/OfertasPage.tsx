    ...
    try {
      ...
      if (etapa && etapa.valor > 0) {
        ...
        if (importeCalculado > Number(etapa.valor)) {
          toast.error(`El importe de la oferta (${importeCalculado.toFixed(2)}) no puede ser mayor al valor de la etapa (${Number(etapa.valor).toFixed(2)})`);
          return;
        }
      }
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 rounded-xl border border-amber-100">
                  ...
                  <p className="font-bold text-amber-900 text-xl">{getMonedaSymbol(detailModal.item.id_moneda)} {Number(detailModal.item.importe || 0).toFixed(2)}</p>
                </div>
