...
function SearchSelect({
  ...
  const selectedItem = selectedId
    ? items.find((i) => getId(i) === selectedId)
    : null;
    ...
    <div className="relative" ref={ref}>
      ...
      {selectedItem ? (
        <div className="mt-1 flex items-center gap-2 px-3 py-2 bg-teal-50 border border-teal-200 rounded-lg">
          <span className="flex-1 text-sm text-teal-800 font-medium truncate">
...
export function FacturasServicioPage() {
  ...
  const [selectedContratoId, setSelectedContratoId] = useState<number | null>(
    contratoParam ? Number(contratoParam) : null,
  );
  ...
  const loadEtapasBySolicitud = async (solicitudId: number) => {
    try {
      ...
      const solicitudes =
        await solicitudesService.getSolicitudesByContrato(contratoId);
  ...
  const loadFacturas = async () => {
    try {
      if (filtroEtapa) {
        const data =
          await facturasServicioService.getFacturasByEtapa(filtroEtapa);
        setFacturas(data);

      } else if (contratoParam) {
        const contratosData = await contratosService.getContratos(0, 1000);
        const contrato = contratosData.find(
          (c: ContratoWithDetails) => c.id_contrato === Number(contratoParam),
        ) as any;
        if (contrato?.solicitudes) {
          ...
          const todasLasFacturas =
            await facturasServicioService.getFacturasServicio(0, 1000);
          const filtradas = todasLasFacturas.filter(
            (f: FacturaServicio) => f.id_etapa && etapaIds.includes(f.id_etapa),
          );
  ...
  }, [view, filtroEtapa, contratoParam]);
    ...
    try {
      if (editingId) {
        ...
        const data: FacturaServicioCreate = {
          id_etapa:
            selectedEtapaId || (etapaParam ? Number(etapaParam) : undefined),
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">
              {currentEtapa
                ...
                : "Gestión de facturas de servicio"}
              ...
              {filteredFacturas.length === 0 ? (
                ...
                filteredFacturas.map((item) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.alcance === "TOTAL" ? "bg-blue-100 text-blue-800" : "bg-orange-100 text-orange-800"}`}
                      >
                        {item.alcance || "N/A"}
                      </span>
                    </TableCell>
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <Button
                        ...
                        onClick={() =>
                          navigate(
                            `/proyectos/pagos-factura-servicio?factura=${item.id_factura_servicio}`,
                          )
                        }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Complete los datos de la factura de servicio
            </p>
          </div>
        </div>
        <Button
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-auto animate-scale-in">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">
                        Factura {detailModal.item.codigo_factura || ""}
                      </h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-xl border border-green-100">
                    ...
                    <p className="font-bold text-green-900 text-xl">
                      ...
                      {(
                        Number(detailModal.item.precio) *
                        Number(detailModal.item.cantidad)
                      ).toFixed(2)}
                    </p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-xl">
                    ...
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${detailModal.item.alcance === "TOTAL" ? "bg-blue-100 text-blue-800" : "bg-orange-100 text-orange-800"}`}
                    >
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-xl border border-blue-100">
                    ...
                    <p className="font-bold text-gray-900">
                      {getMonedaSymbol(detailModal.item.id_moneda)}{" "}
                      {Number(detailModal.item.precio).toFixed(2)}
                    </p>
                  </div>
              ...
              <div className="p-6 border-t border-gray-200 bg-gray-50 flex justify-end gap-2">
                <Button
                  ...
                  onClick={() =>
                    navigate(
                      `/proyectos/pagos-factura-servicio?factura=${detailModal.item?.id_factura_servicio}`,
                    )
                  }
