...
export function ProductosEnLiquidacionPage() {
  ...
  const [detailModal, setDetailModal] = useState<{
    isOpen: boolean;
    item: ProductosEnLiquidacion | null;
  }>({ isOpen: false, item: null });

  const [editingId, setEditingId] = useState<number | null>(null);
  const [formData, setFormData] = useState<
    Partial<ProductosEnLiquidacionCreate>
  >({});
  ...
  const loadProductos = async () => {
    ...
    try {
      ...
      if (filterType === "pendientes") {
        data =
          await productosEnLiquidacionService.getProductosEnLiquidacionPendientes();
      } else if (filterType === "liquidadas") {
        data =
          await productosEnLiquidacionService.getProductosEnLiquidacionLiquidadas();
      } else {
  ...
  const handleSave = async () => {
    try {
      ...
      editingId
        ? await productosEnLiquidacionService.updateProductoEnLiquidacion(
            editingId,
            formData,
          )
        : await productosEnLiquidacionService.createProductoEnLiquidacion(
            formData as ProductosEnLiquidacionCreate,
          );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-gradient-to-br from-red-500 to-rose-600 rounded shadow-lg animate-bounce-subtle">
        ...
        <div className="flex gap-2">
          <Button
            variant={filterType === "pendientes" ? "primary" : "outline"}
            size="sm"
          ...
          <Button
            variant={filterType === "liquidadas" ? "primary" : "outline"}
            size="sm"
            onClick={() => setFilterType("liquidadas")}
          >
            Liquidadas
          </Button>
          <Button
            ...
            <TableBody>
              {loading ? (
                ...
                filteredProductos.map((item) => (
                  <TableRow
                    key={item.id_producto_en_liquidacion}
                  ...
                  >
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.liquidada ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}
                      >
                      ...
                      <div className="flex justify-end gap-2">
                        {!item.liquidada && (
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              handleLiquidar(item.id_producto_en_liquidacion)
                            }
                        ...
                        <Button
                          ...
                          size="icon"
                          onClick={() =>
                            handleDelete(item.id_producto_en_liquidacion)
                          }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    tipo_compra: e.target.value as any,
                  })
                }
              >
                <option value="">Seleccionar</option>
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-auto animate-scale-in">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">
                        {detailModal.item.producto?.nombre || "Producto"}
                      </h3>
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 rounded-md border border-blue-100">
                    ...
                    <p className="font-bold text-blue-900 text-xl">
                      $
                      {Number(
                        detailModal.item.producto?.precio_venta || 0,
                      ).toFixed(2)}
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-md">
                    ...
                    <p className="font-bold text-gray-900">
                      {getTipoLabel(detailModal.item.tipo_compra)}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-md border border-purple-100">
                    ...
                    <p className="font-bold text-gray-900">
                      {new Date(detailModal.item.fecha).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-md">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">
                    ...
                    <p className="font-bold text-gray-900">
                      {detailModal.item.moneda?.nombre || "N/A"}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-md">
                  ...
                  <span
                    className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${detailModal.item.liquidada ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}
                  >
