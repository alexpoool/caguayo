...
export function ClientesPage() {
    ...
    mutationFn: async (data: ClienteCreate) => {
      ...
      if (tipoPersona === "NATURAL") {
        ...
        await clienteTCPService.createClienteTCP({
          ...
          numero_registro_proyecto:
            datosTCPForm.numero_registro_proyecto || undefined,
  ...
  const handleViewDetails = async (cliente: Cliente) => {
    ...
    try {
      if (cliente.tipo_persona === "NATURAL") {
        const data = await clienteNaturalService.getClienteNatural(
          cliente.id_cliente,
        );
        ...
        const data = await clienteJuridicaService.getClienteJuridica(
          cliente.id_cliente,
        );
      ...
      // Cargar cuentas
      const cuentasData = await cuentasService.getCuentasByCliente(
        cliente.id_cliente,
      );
  ...
  const addCuenta = () => {
    if (nuevaCuenta.titular && nuevaCuenta.banco) {
      setCuentas([
        ...cuentas,
        { ...nuevaCuenta, sucursal: nuevaCuenta.sucursal || 0 },
      ]);
      setNuevaCuenta({
        titular: "",
        banco: "",
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  checked={datosNaturalForm.es_trabajador}
                  onChange={(e) =>
                    setDatosNaturalForm({
                      ...datosNaturalForm,
              ...
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  aria-label="En baja"
                  checked={datosNaturalForm.en_baja}
                  onChange={(e) =>
                    setDatosNaturalForm({
                      ...datosNaturalForm,
