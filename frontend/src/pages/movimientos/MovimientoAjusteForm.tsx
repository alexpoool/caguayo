    ...
    <div ref={containerRef} className="relative">
      ...
      <div className="relative">
        ...
        >
          ...
          />
          {selectedItem && (
            <button type="button" onClick={handleClear} className="p-1 mr-1 hover:bg-gray-200 rounded-full transition-colors">
              <X className="h-3.5 w-3.5 text-gray-400" />
            </button>
          )}
          <ChevronDown className={`h-4 w-4 text-gray-400 mr-3 transition-transform flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
        </div>
      </div>
      {renderDropdown()}
    </div>
...
export function MovimientoAjusteForm() {
  ...
  const [productoSeleccionado, setProductoSeleccionado] = useState<ProductoConCantidad | null>(null);
  const [dependenciaOrigen, setDependenciaOrigen] = useState<{ id: number; nombre: string } | null>(
    dependenciaUsuario ? { id: dependenciaUsuario.id_dependencia, nombre: dependenciaUsuario.nombre } : null
  );
  const [stockOrigen, setStockOrigen] = useState(0);
  const [destinos, setDestinos] = useState<Destino[]>([{ id: 1, id_dependencia: null, cantidad: 0 }]);
  ...
  const { data: productos = [], isLoading: isLoadingProductos } = useQuery({
    ...
    queryFn: () => movimientosService.getProductosPorDependencia(dependenciaUsuario!.id_dependencia),
  ...
  const handleDestinoChange = (id: number, field: 'id_dependencia' | 'cantidad', value: number | null) => {
    setDestinos(destinos.map(d => d.id === id ? { ...d, [field]: value ?? 0 } : d));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const destinosValidos = destinos
      ...
      .map(d => ({ id_dependencia: d.id_dependencia as number, cantidad: d.cantidad }));
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">Transferencia de productos entre dependencias</p>
          </div>
        </div>
        <Button variant="outline" onClick={() => navigate('/movimientos')} className="gap-2">
        ...
        <div className="space-y-6">
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SearchableSelect
                  ...
                  value={productoSeleccionado ? Number(productoSeleccionado.id_producto) : null}
                  onChange={(value, item) => setProductoSeleccionado(item as ProductoConCantidad | null)}
                  options={productos}
                  ...
                  getOptionDisplay={(item) => (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-gray-500 text-xs">Stock: {item.cantidad}</span>
                    </div>
                  )}
                  idField="id_producto"
                />
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SearchableSelect
                  ...
                  onChange={(value, item) => setDependenciaOrigen(item ? { id: item.id_dependencia, nombre: item.nombre } : null)}
                ...
                <div>
                  ...
                  <div className="flex items-center h-[46px] mt-1 px-4 border border-gray-200 rounded-lg bg-gray-50">
                    <span className={`text-lg font-bold ${stockOrigen > 0 ? 'text-green-600' : 'text-gray-400'}`}>
          ...
          <Card className="shadow-sm border-gray-200">
            ...
            <CardContent className="p-6">
                  ...
                  <div key={destino.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      <SearchableSelect
                        ...
                        options={dependencias.filter(d => !dependenciaOrigen || d.id_dependencia !== dependenciaOrigen.id)}
                        isLoading={isLoadingDependencias}
                        placeholder="Buscar dependencia..."
                        getOptionLabel={(item) => item.nombre}
                        getOptionValue={(item) => item.id_dependencia}
              ...
              <div className="mt-4">
                <Button type="button" onClick={handleAgregarDestino} variant="secondary" className="gap-2">
              ...
              <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-gray-700">
                      Total a transferir:
                      <span className="font-bold text-green-600 ml-2">{cantidadTotalDestinos} unidades</span>
                    </span>
                  </div>
                ...
                {cantidadTotalDestinos > stockOrigen && (
                  <p className="text-red-600 text-sm mt-2">La cantidad total en destinos excede el stock disponible en origen</p>
                )}
                {dependenciaOrigen && destinos.some(d => d.id_dependencia === dependenciaOrigen.id) && (
                  <p className="text-red-600 text-sm mt-2">No puedes agregar la dependencia origen como destino</p>
                )}
              </div>
            </CardContent>
          </Card>
